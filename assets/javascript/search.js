...
function createNRHELink(name, id, url) {
  return '<p class="links"><i class="bi bi-archive-fill" style="margin-right: 15px;" data-toggle="tooltip" data-placement="left" title="NRHE entry available"></i><a href="http://' + url + '" target="_blank">' + name + ' (Id: ' + id + ') <i class="bi bi-link-45deg"></i></a></p>'
}
...
function displaySearchResults(results, store) {
  ...
  if (results.length) {
    ...
    for (var i = start; i < iterator; i++) { // Iterate over the results
      ...
      appendString += '<p class="info">' + item.location + ' (' + item.country + ') at <a href="https://www.openstreetmap.org/#map=6/' + item.lng + '/' + item.lng + '" target="_blank">' + item.ngr + '</i></a></p>'
