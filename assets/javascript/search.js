    ...
    if (results.length) { // Are there any results?
      ...
      for (var i = 0; i < results.length; i++) { // Iterate over the results
        ...
        if (item.links.ei.availability) {
          appendString += '<p class="value3 mt-sm"><img src="https://historicengland.org.uk/public/src/images/HE-Logo_White.svg" style="margin-right: 15px;height: 16px;vertical-align: text-top;filter: invert(69%) sepia(0%) saturate(6125%) hue-rotate(331deg) brightness(82%) contrast(53%);object-fit: cover;object-position: left;width: 16px;">' + item.links.ei.name + ' (Id: ' + item.links.ei.id + ') <a href="http://' + item.links.ei.url + '" target="_blank"><i class="bi bi-link-45deg"></i></a></p>'
        } else {
